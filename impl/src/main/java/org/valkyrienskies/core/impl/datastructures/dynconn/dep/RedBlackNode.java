 ...
 *
 * For NodeReference, a red-black tree is a binary search tree satisfying the following properties:
 *
...
public abstract class RedBlackNode<N extends RedBlackNode<N>> implements Comparable<N> {
     ...
     */
    private void assertSubtreeIsValidRedBlack(int blackHeight, Set<NodeReference<N>> visited) {
        @SuppressWarnings("unchecked")
        N nThis = (N)this;
        if (left == null || right == null) {
            ...
            return;
        } else if (!visited.add(new NodeReference<N>(nThis))) {
    ...
    public void assertSubtreeIsValidRedBlack() {
        if (isLeaf()) {
            ...
            Set<NodeReference<N>> nodes = new HashSet<NodeReference<N>>();
            int blackHeight = 0;
            @SuppressWarnings("unchecked")
            N node = (N)this;
            while (node != null) {
                if (!nodes.add(new NodeReference<N>(node))) {
                    throw new RuntimeException("The tree contains a repeated non-leaf node");
                }
                if (!node.isRed) {
                    blackHeight++;
            ...
            assertSubtreeIsValidRedBlack(blackHeight, new HashSet<NodeReference<N>>());
        }
    }
